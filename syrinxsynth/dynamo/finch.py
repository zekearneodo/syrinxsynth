...
from syrinxsynth.dynamo import ode

# Parameters of the syrinx and numerical integration
...
# parameters of the vocal tract, oec, et. al.
...
def takens_finch(v, pars):
    # Deprecated. Newer version does not use a dictionary for the parameters, to play nice with numba
...
# List of parameters of the syrinx used by the vector field to integrate
takens_finch_pars = ['gamma', 'alpha_1', 'beta_1', 'Ch_inv', 'Lb_inv', 'Lg_inv', 'Rb', 'Rh', 'dV_ext', 'V_ext']
@jit(nopython=True)
def takens_finch_fast(v, g, a_1, b_1, ch_inv, lb_inv, lg_inv, rb, rh, dv_ext, v_ext):
    gg = g*g
    [x, y, i_1, i_2, i_3] = v

    return np.array([
        y,
        a_1 * gg + b_1 * gg * x - gg * x * x * x - g * x * x * y + g * x * x - g * x * x * y,
        i_2,
        -lg_inv * ch_inv * i_1 - rh * (lb_inv + lg_inv) * i_2 + (lg_inv * ch_inv - rb * rh * lb_inv * lg_inv) * i_3 
        + lg_inv * dv_ext + rh * lg_inv * lb_inv * v_ext,
        - lb_inv/lg_inv * i_2 - rb * lb_inv * i_3 + lb_inv*v_ext
    ])

#@jit
def finch(pars: dict, int_par_stream: np.array, x_0: np.array=None) -> np.array:
    """
    Generates a synthetic vocalization of zebra finch for the stream of forcing parameters in int_par_stream,
    with morphology contained in pars
    ...
    for step in np.arange(pars['max_tau'], pars['max_tau'] + total_steps):
        ...
        if sampling_t == 0:
            ...
            pre_out[t_samples, 0] = x[4] # output of the beak
            pre_out[t_samples, 1] = x[0] # output of the syringeal valve (input of tract)
        ...
        # Iterate integration of the source:
        # parameters from dictionary to tuple for fast integration using numba
        #ode_par_tuple = tuple([pars['sys'][k] for k in takens_finch_pars])
        # run the integration step
        #x = ode.rk4(takens_finch_fast, x, d_t, *ode_par_tuple)
        x = ode.rk4(takens_finch, x, d_t, pars['sys'])

        # t+= d_t #don't advance time here; rather advance t_fractional when sampling
